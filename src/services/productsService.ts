...
} from '@/types/productsResponse.ts'
...
export default {
  ...
  async updateProductById(id: number, productUpdate: IProductUpdate) {
    try {
      const endpoint = `${API_ENDPOINT_NAMES.PRODUCTS}/${id}`
      const response = await api.put<IApiResponse<IProduct>>(endpoint, productUpdate, {})
  ...
  async setImageByProductId(id: number, file: File) {
    try {
      const endpoint = `${API_ENDPOINT_NAMES.PRODUCTS}/${id}/images`
      const response = await api.put<IApiResponse<IUploadImageResponse>>(endpoint, file, {
